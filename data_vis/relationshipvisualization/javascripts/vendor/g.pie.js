Raphael.fn.g.piechart = function (cx, cy, r, values, opts) {
    ...
    if (len == 1) {
        ...
        for (i = 0; i < len; i++) {
            ...
            var path = sector(cx, cy, r, angle, angle -= 360 * values[i] / total);      //changes here
            var p = this.path(opts.init ? ipath : path).attr({fill: opts.colors && opts.colors[i] || this.g.colors[i] ||"#666", stroke: opts.stroke || "#fff", "stroke-width": (opts.strokewidth == null ? 1 : opts.strokewidth), "stroke-linejoin": "round", "fill-opacity":opts.opacity[i]});
            p.value = values[i];
